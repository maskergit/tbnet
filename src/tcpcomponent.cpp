...
TCPComponent::~TCPComponent() {
    TBSYS_LOG(DEBUG, "TCPComponent::~TCPComponent");
    if (_connection) {
...
bool TCPComponent::init(bool isServer) {
    TBSYS_LOG(DEBUG, "TCPComponent::init");
...
bool TCPComponent::socketConnect() {
    TBSYS_LOG(DEBUG, "TCPComponent::socketConnect");
    ...
    if (_socket->connect()) {
        ...
        if (error == EINPROGRESS || error == EWOULDBLOCK) {
            ...
            _socket->close();
            _state = TBNET_CLOSED;
            TBSYS_LOG(ERROR, "connect to %s failed, %s(%d)", _socket->getAddr().c_str(), strerror(error), error);
            return false;
        }
    }
    return true;
...
void TCPComponent::checkTimeout(int64_t now) {
    TBSYS_LOG(DEBUG, "TCPComponent::checkTimeout(%ld)", now);
