...
bool Socket::connect() {
    ...
    TBSYS_LOG(INFO, "open connection, sock fd=%d, addr=%s", _socketHandle, getAddr().c_str());
...
void Socket::close() {
    if (_socketHandle != -1) {
        TBSYS_LOG(INFO, "close connection, sock fd=%d, addr=%s", _socketHandle, getAddr().c_str());
        ::close(_socketHandle);
        _socketHandle = -1;
    }
}
