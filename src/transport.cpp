...
bool Transport::start() {
    TBSYS_LOG(DEBUG, "Transport::start");
 ...
 */
bool Transport::stop() {
    TBSYS_LOG(DEBUG, "Transport::stop");
    _stop = true;
    return true;
}
 ...
 */
bool Transport::wait() {
    TBSYS_LOG(DEBUG, "Transport::wait");
...
void Transport::eventLoop(SocketEvent *socketEvent) {
    TBSYS_LOG(DEBUG, "transport::eventLoop()");
    ...
    while (!_stop) {
        ...
        if (cnt < 0) {
            TBSYS_LOG(INFO, "get events failed: %s(%d)\n", strerror(errno), errno);
        }
...
IOComponent *Transport::listen(const char *spec, IPacketStreamer *streamer, IServerAdapter *serverAdapter) {
    ...
    if (strcasecmp(args[0], "tcp") == 0) {
        TBSYS_LOG(DEBUG, "Transport::listen()");
...
Connection *Transport::connect(const char *spec, IPacketStreamer *streamer, bool autoReconn) {
    TBSYS_LOG(DEBUG, "Transport::connect");
    ...
    if (strcasecmp(args[0], "tcp") == 0) {
        ...
        if (!socket->setAddress(host, port)) {
            delete socket;
            TBSYS_LOG(ERROR, "setAddress failed: %s:%d, %s", host, port, spec);
            return NULL;
        }
        ...
        if (!component->init()) {
            TBSYS_LOG(ERROR, "init TCPComponent failed: %s:%d", host, port);
            delete component;
            return NULL;
        }
...
void Transport::addComponent(IOComponent *ioc, bool readOn, bool writeOn) {
    ...
    _iocsMutex.lock();
    if (ioc->isUsed()) {
        TBSYS_LOG(ERROR, "already added ioc: %p", ioc);
        _iocsMutex.unlock();
        return;
    }
    // 加入iocList上
...
void Transport::destroy() {
    TBSYS_LOG(DEBUG, "Transport::destroy");
