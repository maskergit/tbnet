...
Connection::~Connection() {
    TBSYS_LOG(DEBUG, "Connection::~Connection");
...
void Connection::disconnect() {
    TBSYS_LOG(DEBUG, "Connection::disconnect()");
...
bool Connection::postPacket(Packet *packet, IPacketHandler *packetHandler, void *args, bool noblocking) {
    TBSYS_LOG(DEBUG, "Connection::postPacket");
    ...
    if (_streamer->existPacketHeader()) {           // 存在包头
        ...
        if (_isServer) {
            ...
            // channel没找到
            if (channel == NULL) {
                TBSYS_LOG(WARN, "alloc channel failed, id: %u", chid);
                return false;
            }
...
bool Connection::checkTimeout(int64_t now) {
    TBSYS_LOG(DEBUG, "Connection::checkTimeout(%ld)", now);
