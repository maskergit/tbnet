...
public:
    ...
    HPRetCode handlePacket(Packet *packet, void *args) {
        TBSYS_LOG(DEBUG, "ClientEchoPacketHandler::handlePacket");
...
void EchoClient::start(int conncount) {
    TBSYS_LOG(DEBUG, "EchoClient::start");
    ...
    int sendcount = 0;
    int pid = getpid();
    TBSYS_LOG(DEBUG, "send start, PID: %d", pid);
...
int main(int argc, char *argv[]) {
    ...
    TBSYS_LOG(DEBUG, "sendcount %d, conncount %d", sendcount, conncount);

    EchoClient echoServer(argv[1]);
    echoServer.start(conncount);
    int64_t endTime = tbsys::CTimeUtil::getTime();

    TBSYS_LOG(DEBUG, "speed: %d tps, agv size: %d, encode_count: %d\n", 
